...
1. **The Gigantic Class Purge**:
   - Extract the `RenderGrid`, `RenderMeasureLines`, and `DrawInputNotes` logic into a new `[NEW] Core/Rendering/CommonRenderer.cs` utility.
   - Move all countdown/timer logic into a `[NEW] Core/Services/PlaybackTimerService.cs`.
2. **Binary Packaging**:
