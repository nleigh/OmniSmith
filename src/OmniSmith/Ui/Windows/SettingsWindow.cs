...
using static OmniSmith.Settings.CoreSettings;
using static OmniSmith.Settings.AudioDriverManager;
using static OmniSmith.Settings.DevicesManager;
using static OmniSmith.Settings.MidiPathsManager;
using static OmniSmith.Settings.SoundFontsPathsManager;
using OmniSmith.Ui.Helpers;
using OmniSmith.Core;
using OmniSmith.Core.FileDialogs;
...
public class SettingsWindow : ImGuiWindow
{
    ...
    protected override void OnImGui()
    {
        ...
        foreach (var path in MidiPaths.ToList())
        {
            ...
            int nFiles = 0;
            foreach (var songFile in Directory.EnumerateFiles(path, "*.*", SearchOption.AllDirectories))
            {
        ...
        if (ImGui.Button($"{FontAwesome6.TrashCan} Clear Metadata Cache", ImGuiUtils.FixedSize(new Vector2(250, 40))))
        {
            ...
            string cacheDir = Path.Combine(KnownFolders.RoamingAppData.Path, "OmniSmith", "Cache", "Thumbnails");
